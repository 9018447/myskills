...
如果用户没有指定 Agent，必须先询问用户，由用户明确指定后才能执行，不得自行选择 Agent。

Tickets 原则上依次执行。每个 Ticket 必须单独通过 `/acpx` 开启一个新的 Agent 会话，一票一次，不得将多个 Tickets 堆入同一个会话，以避免阻塞、上下文污染和上下文膨胀。

当用户已经指定的 Agent 出现额度耗尽、不可用、执行失败或无法正常工作时，才允许进入候补分发流程。

候补分发必须根据当前时间决定。工作时间定义为 08:00–18:00；非工作时间定义为 18:00–次日 08:00。

工作时间：kimi → claude
非工作时间：zcode → claude → dsh

其中，kimi 是工作时间的首选 Agent，zcode 是非工作时间的首选 Agent；claude 作为二者的统一候补；dsh 仅作为非工作时间的最后候补。

在分发给 zcode 或 dsh 之前，必须先查询当前时间。只有当前时间处于 18:00–次日 08:00 时，才允许调用 zcode 或 dsh。不得仅凭上下文中的时间、历史时间或主观判断推断当前时间。

如果用户指定了明确的 Agent，则优先遵循用户指定，不得因为默认优先级而擅自更换；只有该 Agent 确实无法继续工作时，才进入上述候补链。
