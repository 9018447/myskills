...
**例外——长时间运行票默认派给 pi，无需询问、无需用户指定。** 判定标准是票面的运行预算字段：凡标注 `含 N 次真实运行` 或明示单次/合计时长超过约 20 分钟的运行票（如回归验收、真实语料批跑），一律直接派给 pi，不进入询问流程，也不受工作时间窗与候补链约束。用户在参数里明确为该票指定了其他 Agent 时，用户指定优先；pi 不可用时按候补链处理。
...
- 每票先写派发 prompt 到文件（包含下述 6 点告知事项），再后台启动 acpx，日志落到文件；`--timeout` 按票面的**运行预算**设置（预算为 `含 N 次真实运行` 的票须确认已与实现拆开，超上限时拆票而不是调大超时硬扛）。
...
- 派发被中途杀死（超时、手动停止）后重派时：续作 prompt 先要求审计盘上现状——`pnpm run check` 跑到绿、复核半成品符合票面则留用不推倒重来，再补完剩余验收；并在交付记录里注明中断与续作的事实。
