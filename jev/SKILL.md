...
3. **Formulate:** use `choice` for mutually exclusive paths, `noul` for independent
   ...
   **Fact — `criteria` field shape (verified against the CLI validator; wrong
   shapes fail at request build):**

   - `choice`: `criteria` is **required** and must be an **object** mapping
     label → one-line description (`{"q_scope": "收编范围：全部还是只收主技能"}`),
     2–255 entries. A JSON array of labels is rejected, and so is putting the
     candidates only inside `instructions`.
   - `noul`: `criteria` is **optional**; if present it must be exactly
     `{"true": ..., "false": ...}`.
   - `score`: `criteria` is a **list** of 2–10 ordered level descriptions.
