...
5. **Install** the wrapper into the skill's scripts directory:
   ```bash
   uvx mcp2cli bake install myapi --dir ~/homeconfig/skills/mcp2cli_myapi/scripts/
   ```

   **Create a SKILL.md** in `~/homeconfig/skills/mcp2cli_myapi/` that teaches another AI agent how to use this API. The SKILL.md must go beyond `--help` output — focus on knowledge that can only be learned through testing and reading documentation.
   ...
   **Core Workflow** (discovery + execution):
   ...
   ~/homeconfig/skills/mcp2cli_myapi/scripts/myapi --list
   # Get help for a command
   ~/homeconfig/skills/mcp2cli_myapi/scripts/myapi <command> --help
   # Run a command
   ~/homeconfig/skills/mcp2cli_myapi/scripts/myapi <command> --param value --pretty
   ```
   ...
   **Output Processing** — use `--pretty` for readable JSON, `--head` to limit results, or pipe to `jq` for filtering:
   ...
   ~/homeconfig/skills/mcp2cli_myapi/scripts/myapi list-records --pretty
   # Limit large datasets
   ~/homeconfig/skills/mcp2cli_myapi/scripts/myapi list-records --head 5
   # Filter with jq (pipe)
   ~/homeconfig/skills/mcp2cli_myapi/scripts/myapi list-records | jq '.[].name'
   ```
   ...
   **Export Formats** (if the API supports multiple output types):
   ...
   - For binary formats: `~/homeconfig/skills/mcp2cli_myapi/scripts/myapi export --format xlsx --raw > output.xlsx`
