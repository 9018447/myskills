...
Works best after a previous search call, which already extracts `fullTextUrl` and `href` for each result. If those are available, use them directly.
...
If no previous search context is available, search for the paper first.
...
- Uses 1-2 tool calls: `evaluate_script` (if already on results page) or `navigate_page` + `evaluate_script`
