...
交付课程时，优先用 lavish-axi 打开可标注的审阅副本（用户可在页面上框选元素/划选文字写反馈）：

1. 拷贝课程到 `.lavish/<课名>.html`，并把其引用的本地资源（`assets/`、被链接的参考文档等）拷到 `.lavish/` 同目录下，改写相对路径——lavish 服务器以文件所在目录为根，`../` 引用会 404。
2. `lavish-axi .lavish/<课名>.html` 打开会话，然后 `lavish-axi poll <文件>` 等待反馈（可用 harness 的后台作业机制挂起，结果会自动送达；不要用 nohup/`&`）。
3. 收到反馈后**只改 `./lessons/` 正本**，再重新拷贝生成审阅副本。`.lavish/` 是一次性副本，永远不直接编辑。
4. lavish-axi 不可用时，退回 `xdg-open <课程文件>`。
